...
fig_x_axis <- tickr(data.frame(breaks = 1990:2018), breaks, 2)
...
metadata <- read.csv("./data/fishery_metadata.csv") # fishery codes for target species, year, and mgt area
...
obs_dump %>% 
  left_join(metadata, by = c("fishery" = "fishery_code")) %>%
  f_directed_fishery(., Tspecies) %>%
  mutate(component = case_when(sex == 1 ~ "male", 
                               clutch == -9 ~ "immatureF",
...
potsum %>% 
  left_join(metadata, by = c("fishery" = "fishery_code")) %>%
  f_directed_fishery(., Tspecies) %>%
  mutate(tot_male = sublegal + tot_legal) %>% 
  filter(directed == T) -> potsum2
